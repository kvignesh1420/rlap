...
class InfoNCEBatched(Loss):
    ...
    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        ...
        for i in range(num_batches):
            ...
            batch_log_prob = batch_sim - torch.log(
                batch_exp_sim.sum(dim=1, keepdim=True)
            )
...
def main():
    ...
    augmentors = {
        ...
        "RandomWalkSubgraph": [
            A.Compose(
                [
                    A.RWSampling(num_seeds=int(fraction1 * num_nodes), walk_length=10),
                    A.FeatureMasking(pf=0.3),
                ]
            ),
            A.Compose(
    ...
    contrast_model = DualBranchContrast(
        loss=InfoNCEBatched(tau=0.4, batch_size=1024),
        mode=args.mode,
