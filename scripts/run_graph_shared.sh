...
mkdir -p results/final/graph/shared/g2g/infonce
...
FILE="results/final/graph/shared/g2g/infonce/$aug-$dataset-$num_layers-$lr-$wd-$dim-$mode.txt"
if test -f "$FILE"; then
    echo "$FILE exists."
else
for frac1 in ${FRAC1[@]}
...
python3 graph_shared.py $aug $dataset $num_layers $lr $wd $dim $mode $frac1 $frac2 >> "$FILE"
done # frac1
done # frac2
fi
done # modes
